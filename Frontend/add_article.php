...
$articel_name=$article['name'];
$source_atl="articles/";
$alt_path =$source_atl.$articel_name;
move_uploaded_file($article['tmp_name'], $alt_path);
...
$query = "INSERT INTO articles (title,photo,uploadDate,Description,article,faculties_id,student_name,student_id)
 VALUES (:title,:photo,:uploadDate,:message,:article,:faculties_id,:student_name,:student_id)";
if ($query) {
	try 
	{
			$mail->isSMTP();
	      ...
	      // Gmail ID which you want to use as SMTP server
			$mail->Username = 'auroromagazine@gmail.com';
	      // Gmail Password
			$mail->Password = 'auroromagazine123';
			$mail->SMTPSecure = 'tls';
			$mail->Port = 587;

	      // Email ID from which you want to send the email
			$mail->setFrom('auroromagazine@gmail.com','Otika');
...
$stmt->bindParam(':uploadDate', $tdyDate);
$stmt->bindParam(':message', $message);
$stmt->bindParam(':article', $alt_path);
$stmt->bindParam(':student_name', $stuName);
$stmt->bindParam(':faculties_id', $faculty_id);
